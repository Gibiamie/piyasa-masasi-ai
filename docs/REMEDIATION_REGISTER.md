...
|---|---|---|---|---|---|---|---|
| MIC-P0-001 | P0 | Concentration weight alone triggers automatic "DENGELE/AZALT" lot-sale instructions | `legacy-import/mic/app-main.js` → `decision()` | Planned: replace single sell output with 4 independent outputs (thesis / income health / concentration / user action); weight breach alone must yield `KONSANTRASYON UYARISI — SATIŞ SİNYALİ DEĞİLDİR`, never a lot count, unless user explicitly requests the scenario calculator | pending | pending | In progress |
| MIC-P0-002 | P0 | Missing price treated as zero value, inflating other weights and enabling bad sell decisions | `legacy-import/mic/price-integrity-v18.js` → `portfolioStats()` | Planned: any missing/untimestamped price locks weight + concentration + lot calculations and shows `PORTFÖY AĞIRLIĞI HESAPLANAMADI`; stored/manual price allowed only with visible timestamp + "estimated" label | pending | pending | In progress |
