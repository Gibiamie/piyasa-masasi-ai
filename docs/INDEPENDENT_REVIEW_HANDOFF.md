...
2. **GitHub Pages/Fastly CDN edge-cache staleness, confirmed persistent across a redeploy — the live site is not currently a reliable source of truth for verification.** Observed twice: once before pushing `e673eb9`, and again ~15 minutes after that commit was pushed, `pages-deploy.yml` completed successfully, and direct `curl` confirmed the origin serves corrected content. Both times, live-browser Playwright runs (Chromium + WebKit) got the identical stale response (`DENGELE / AZALT` instead of `KONSANTRASYON UYARISI`; a 404 for `sw.js?v=5`, code already removed) while `curl` against the same URL at the same time got the correct content. This survived past the `Cache-Control: max-age=600` window. Not an application defect — confirmed correct at every layer this repo controls (CI on Linux: 123/123 pass; local: 82/82 pass; direct HTTP against origin: correct). A reviewer relying on `https://gibiamie.github.io/piyasa-masasi-ai/` directly may see stale, pre-fix behavior through no fault of the code. Recommend checking again after a longer interval, or reviewing via `git diff` + the local/CI suites instead of trusting the live URL alone until this clears.
