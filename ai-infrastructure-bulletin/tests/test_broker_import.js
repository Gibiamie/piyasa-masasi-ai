...
async function run() {
  ...
  const payload = Importer.holdingsToPortfolio(parsedPdf.holdings, {
    date: parsedPdf.statementDate,
    currency: "TRY",
    sourceLabel: "Synthetic broker test"
  });
  assert.equal(payload.transactions.length, 2);
  assert.equal(Object.keys(payload.manualPrices).length, 2);
  const portfolio = PortfolioEngine.calculate(payload.transactions, payload.manualPrices);
  assert.equal(portfolio.isValid, true);
  assert.equal(portfolio.openHoldings.length, 2);
  close(portfolio.totalsByCurrency.TRY.remainingCost, 1550);
  close(portfolio.totalsByCurrency.TRY.marketValue, 1650);
  close(portfolio.totalsByCurrency.TRY.unrealizedPnl, 100);
