...
const chartController = fs.readFileSync("ai-infrastructure-bulletin/chart-controller-v27.js", "utf8");
...
for (const [name, source] of [["bootstrap", bootstrap], ["workspace", workspace], ["intraday", intraday], ["live", live], ["chartController", chartController], ["research", research], ["serviceWorker", serviceWorker]]) {
...
assert.match(bootstrap, /chart-controller-v27\.js/);
...
assert.match(bootstrap, /2026\.08\.09\.27/);
...
assert.match(chartController, /dailyHistoryUnavailable/);
assert.match(chartController, /activateIntradayFallback/);
assert.match(chartController, /data-interval="1m"/);
assert.match(chartController, /pm-market-source-change/);
assert.match(research, /PiyasaMarketWorkspace/);
assert.match(serviceWorker, /piyasa-masasi-workspace-v27/);
assert.match(serviceWorker, /chart-controller-v27\.js/);
assert.doesNotMatch(serviceWorker, /combineMarketIntegration|BIST_WIDGET_GUARD|US_NATIVE_INTRADAY/);

console.log("market-core-architecture: v27 resilient chart fallback validated");
